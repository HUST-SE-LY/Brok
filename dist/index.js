...
async function runBatch(ats) {
    const agent = await getAgent();
    const logger = new FileCallbackHandler(process.env.LANGCHAIN_LOG_PATH || 'logs/langchain.log');
    for (const at of ats) {
        const result = await agent.invoke({ messages: [{ role: 'user', content: at }] }, { callbacks: [logger] });
        console.log(result);
