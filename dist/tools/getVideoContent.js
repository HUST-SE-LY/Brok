...
import z from 'zod';
dotenv.config();
const getVideoBaseInfo = async (params) => {
    const { avid } = params;
    if (!avid) {
        throw new Error('avid is required');
    }
    console.log('avid:', avid);
    const response = await axios.get(`https://api.bilibili.com/x/web-interface/view?aid=${avid}`, {
        headers: {
...
export const getVideoTextContent = async (avid) => {
    if (!avid) {
        throw new Error('avid is required');
    }
    ...
    if (cid) {
        ...
        const response = await axios.get(`https://api.bilibili.com/x/player/wbi/playurl?${wbiParams}`, {
            headers: {
                Cookie: `SESSDATA=${process.env.SESSDATA}`,
            },
        });
