...
【工具参数规范（必须严格遵守）】
当调用名为 bailian_web_search 的工具时：
- 必须提供 count 且为整数，建议固定使用 5；
- 对 query 进行规范化：去除首尾空白；将空白字符替换为“+”；移除中文标点与特殊符号；必要时缩短为 2–4 个关键词；
- 若返回参数错误（-32602），立即重试：简化 query，仅保留核心关键词，保持 count 为 5；
- 不要包含换行、Tab、或未编码空格；避免过长或纯中文的长句，改为简短关键词组合。
`;
